...
public interface SliceInterval {
    ...
    public abstract class Abstract implements SliceInterval {
        ...
        public double drawFromInterval(Likelihood jointDensity, double cutoffDensity, double width) {
            double x0 = variable.getValue(0);
            Interval interval = constructInterval(jointDensity, x0, cutoffDensity, width);
            double x1 = x0;
            boolean found = false;
            while (!found) {
                x1 = MathUtils.uniform(interval.lower, interval.upper);
                if (cutoffDensity < evaluate(jointDensity, x1) && test(jointDensity, x0, x1,
                        cutoffDensity, width)) {
                    found = true;
                } else {
        ...
        protected double evaluate(Likelihood jointDensity, double x) {
            variable.setValue(0, x);
            return sliceSampler.evaluate(jointDensity, 1.0);
        }
    ...
    public class SteppingOut extends Abstract {
        ...
        public Interval constructInterval(Likelihood jointDensity, double x0,
            ...
            int K = (m - 1) - J;
            while (J > 0 && cutoffDensity < evaluate(jointDensity, L) ) {
                L -=  w;
                J--;
            }
            while (K > 0 && cutoffDensity < evaluate(jointDensity, R) ) {
                R +=  w;
                K--;
            }
    ...
    public class Doubling extends Abstract {
        ...
        public Interval constructInterval(Likelihood jointDensity, double x0,
         ...
         protected boolean test(Likelihood jointDensity, double x0, double x1,
                               double cutoffDensity, double width) {
             // TODO
            return true;
        }
